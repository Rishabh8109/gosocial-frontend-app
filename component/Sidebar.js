import Link from 'next/link';
import styles from '../styles/Sidebar.module.css';
import Image from 'next/image';
import {CgFeed} from 'react-icons/cg';
import {HiUserGroup} from 'react-icons/hi';
import {MdEventAvailable} from 'react-icons/md';
import {MdOndemandVideo} from 'react-icons/md';
import {BiImage} from 'react-icons/bi';
import {BsFileEarmarkText} from 'react-icons/bs';
import {AiOutlineShop} from 'react-icons/ai';

import ListItem from './ListItem';

function Sidebar() {
   const userImage = "data:image/jpeg;base64,"
   const myLoader = ({userImage}) => {
    return userImage
  }
   return (
        <div className={styles.left}>
          <div className={styles.profileInfo}>
             <Image 
               loader={myLoader}
               src={userImage}
               width={50}
               height={50}
               alt="Picture of the author"
             />  
           <div>
            <p>Ahamad Nu Fawadr</p>
            <p>@fawait</p>
           </div>
          </div>
          <ul>
            <ListItem to="/" icon={<CgFeed className={styles.icon} />} text={"Feed"}/>
            <ListItem to="/Friends" icon={<HiUserGroup  className={styles.icon} />} text={"Friends"}/>
            <ListItem to="/Friends" icon={<MdEventAvailable  className={styles.icon} />} text={"Event"}/>
            <ListItem to="/Friends" icon={<MdOndemandVideo  className={styles.icon} />} text={"Watch video"}/>
            <ListItem to="/Friends" icon={<BiImage  className={styles.icon} />} text={"Photos"}/>
            <ListItem to="/Friends" icon={<BsFileEarmarkText  className={styles.icon} />} text={"File"}/>
            <ListItem to="/Friends" icon={<AiOutlineShop  className={styles.icon} />} text={"Marketplace"}/>
          </ul>        
        </div>
    )
}

export default Sidebar;
